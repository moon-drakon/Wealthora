...
| Interface substitution | Voice capture depends on `SpeechRecognitionProvider`; the local Windows implementation and unconfigured fallback can be substituted without changing the dialog. | `voice/SpeechRecognitionProvider.java`, `voice/WindowsOfflineSpeechRecognitionProvider.java`, `ui/voice/VoiceQuickEntryDialog.java` | Open Voice Quick Entry and explain how the GUI calls the interface. |
| Composition | `LocalDesktopAuthService` composes repositories and password/recovery services to create, authenticate, and recover local accounts. | `auth/local/LocalDesktopAuthService.java`, `auth/PasswordService.java`, `auth/RecoveryAnswerService.java` | Create a user, then show that its password and recovery answer are stored only as hashes. |
