...
No speech provider or credential is bundled. The application therefore reports the provider as unconfigured and offers **Type or paste a transaction command** as a fully working fallback. Speech and manual input share the same multilingual parser. Language choices are Auto, English, বাংলা, and Banglish / Mixed. Auto is only a provider request and is not presented as detected language when no capable provider exists. Microphone audio is never stored, and cancelling a draft has no financial effect. Provider, microphone, language, privacy, and manual-parser controls are shown under **Settings > Voice Entry**.
